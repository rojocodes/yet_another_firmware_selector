...
function updatePackageList(target) {
  ...
  fetch(config.asu_url + '/openwrt-json/' + target + '/index.json')
  .then(response => response.json())
  .then(all_packages => {
    setupAutocompleteList($('packages'), all_packages, true, _ => {}, textarea => {
      // make package list unique und limit to available packages
      textarea.value = split(textarea.value)
        .filter((value, index, self) => self.indexOf(value) === index)
        .filter((value, index) => all_packages.indexOf(value) !== -1)
        .join(' ');
    });
  });
}
